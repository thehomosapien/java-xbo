...
# Function: to start, stop and restart java-xbo.
...
JAR_FILE_PATH="./build/libs/java-xbo.jar"
PID_FILE_PATH="java-xbo.pid"
LOG_FILE_PATH="java-xbo.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start java-xbo."
        else
            echo "Failed to start java-xbo."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop java-xbo."
        else
            echo "Failed to stop java-xbo."
        fi
    ;;
    restart)
