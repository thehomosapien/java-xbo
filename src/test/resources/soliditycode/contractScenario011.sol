...
contract KittyBase is KittyAccessControl {
    ...
    event Birth(address owner, uint256 kittyId, uint256 maxboId, uint256 sireId, uint256 genes);
    ...
    struct Kitty {
        ...
        // timer (for maxbos) as well as the siring cooldown.
        ...
        // Set to the ID of the sire cat for maxbos that are pregnant,
        ...
        // of whether this cat is acting as maxbo or sire.
        ...
        // (i.e. max(maxbo.generation, sire.generation) + 1)
        uint16 generation;
    }
    ...
    ///  breeding action, called "pregnancy time" for maxbos and "siring cooldown"
    ...
    ///  creature that is both maxbo and sire... to itself! Has an invalid genetic code.
    ...
    /// @param _maxboId The kitty ID of the maxbo of this cat (zero for gen0)
        ...
        returns (uint)
    {
        ...
        require(_maxboId == uint256(uint32(_maxboId)));
...
contract KittyBreeding is KittyOwnership {
    ...
    ///  timer begins for the maxbo.
    event Pregnant(address owner, uint256 maxboId, uint256 sireId, uint256 cooldownEndBlock);
    ...
    /// @dev Check if a sire has authorized breeding with this maxbo. True if both sire
    ///  and maxbo have the same owner, or if the sire has given siring permission to
    ///  the maxbo's owner (via approveSiring()).
    function _isSiringPermitted(uint256 _sireId, uint256 _maxboId) internal view returns (bool) {
        address maxboOwner = kittyIndexToOwner[_maxboId];
        address sireOwner = kittyIndexToOwner[_sireId];

        // Siring is okay if they have same owner, or if the maxbo's owner was given
        // permission to breed with this sire.
        return (maxboOwner == sireOwner || sireAllowedToAddress[_sireId] == maxboOwner);
    }
    ...
    ///  period has passed.
    function _isReadyToGiveBirth(Kitty memory _maxbo) private view returns (bool) {
        return (_maxbo.siringWithId != 0) && (_maxbo.cooldownEndBlock <= uint64(block.number));
    }
    ...
    /// @dev Internal check to see if a given sire and maxbo are a valid mating pair. DOES NOT
    ///  check ownership permissions (that is up to the caller).
    /// @param _maxbo A reference to the Kitty struct of the potential maxbo.
    /// @param _maxboId The maxbo's ID.
    /// @param _sire A reference to the Kitty struct of the potential sire.
    /// @param _sireId The sire's ID
    function _isValidMatingPair(
        Kitty storage _maxbo,
        uint256 _maxboId,
        Kitty storage _sire,
        uint256 _sireId
    )
        ...
        returns(bool)
    {
        ...
        if (_maxbo.maxboId == _sireId || _maxbo.sireId == _sireId) {
            return false;
        }
        if (_sire.maxboId == _maxboId || _sire.sireId == _maxboId) {
            return false;
        }

        // We can short circuit the sibling check (below) if either cat is
        // gen zero (has a maxbo ID of zero).
        if (_sire.maxboId == 0 || _maxbo.maxboId == 0) {
            return true;
        }

        // Kitties can't breed with full or half siblings.
        if (_sire.maxboId == _maxbo.maxboId || _sire.maxboId == _maxbo.sireId) {
            return false;
        }
        if (_sire.sireId == _maxbo.maxboId || _sire.sireId == _maxbo.sireId) {
            return false;
        }
    ...
    /// @dev Internal check to see if a given sire and maxbo are a valid mating pair for
    ///  breeding via auction (i.e. skips ownership and siring approval checks).
    function _canBreedWithViaAuction(uint256 _maxboId, uint256 _sireId)
        internal
        view
        returns (bool)
    {
        Kitty storage maxbo = kitties[_maxboId];
        Kitty storage sire = kitties[_sireId];
        return _isValidMatingPair(maxbo, _maxboId, sire, _sireId);
    }
    ...
    /// @param _maxboId The ID of the proposed maxbo.
    /// @param _sireId The ID of the proposed sire.
    function canBreedWith(uint256 _maxboId, uint256 _sireId)
        external
        view
        returns(bool)
    {
        ...
        Kitty storage sire = kitties[_sireId];
        return _isValidMatingPair(maxbo, _maxboId, sire, _sireId) &&
            _isSiringPermitted(_sireId, _maxboId);
    }

    /// @dev Internal utility function to initiate breeding, assumes that all breeding
    ///  requirements have been checked.
    function _breedWith(uint256 _maxboId, uint256 _sireId) internal {
        // Grab a reference to the Kitties from storage.
        Kitty storage sire = kitties[_sireId];
        Kitty storage maxbo = kitties[_maxboId];

        // Mark the maxbo as pregnant, keeping track of who the sire is.
        maxbo.siringWithId = uint32(_sireId);

        // Trigger the cooldown for both parents.
        _triggerCooldown(sire);
        _triggerCooldown(maxbo);

        // Clear siring permission for both parents. This may not be strictly necessary
        // but it's likely to avoid confusion!
        delete sireAllowedToAddress[_maxboId];
        ...
        emit Pregnant(kittyIndexToOwner[_maxboId], _maxboId, _sireId, maxbo.cooldownEndBlock);
    }

    /// @notice Breed a Kitty you own (as maxbo) with a sire that you own, or for which you
    ///  have previously been given Siring approval. Will either make your cat pregnant, or will
    ///  fail entirely. Requires a pre-payment of the fee given out to the first caller of giveBirth()
    /// @param _maxboId The ID of the Kitty acting as maxbo (will end up pregnant if successful)
    /// @param _sireId The ID of the Kitty acting as sire (will begin its siring cooldown if successful)
    function breedWithAuto(uint256 _maxboId, uint256 _sireId)
        external
        payable
        whenNotPaused
    {
        ...
        // Caller must own the maxbo.
        require(_owns(msg.sender, _maxboId));

        // Neither sire nor maxbo are allowed to be on auction during a normal
        // breeding operation, but we don't need to check that explicitly.
        // For maxbo: The caller of this function can't be the owner of the maxbo
        ...
        // Check that maxbo and sire are both owned by caller, or that the sire
        // has given siring permission to caller (i.e. maxbo's owner).
        // Will fail for _sireId = 0
        require(_isSiringPermitted(_sireId, _maxboId));

        // Grab a reference to the potential maxbo
        Kitty storage maxbo = kitties[_maxboId];

        // Make sure maxbo isn't pregnant, or in the middle of a siring cooldown
        require(_isReadyToBreed(maxbo));
        ...
        _breedWith(_maxboId, _sireId);
    }

    /// @notice Have a pregnant Kitty give birth!
    /// @param _maxboId A Kitty ready to give birth.
    ...
    ///  to the current owner of the maxbo. Upon successful completion, both the maxbo and the
    ///  new kitten will be ready to breed again. Note that anyone can call this function (if they
    ///  are willing to pay the gas!), but the new kitten always goes to the mother's owner.
    function giveBirth(uint256 _maxboId)
        external
        whenNotPaused
        returns(uint256)
    {
        // Grab a reference to the maxbo in storage.
        Kitty storage maxbo = kitties[_maxboId];

        // Check that the maxbo is a valid cat.
        require(maxbo.birthTime != 0);

        // Check that the maxbo is pregnant, and that its time has come!
        require(_isReadyToGiveBirth(maxbo));

        // Grab a reference to the sire in storage.
        uint256 sireId = maxbo.siringWithId;
        Kitty storage sire = kitties[sireId];

        // Determine the higher generation number of the two parents
        uint16 parentGen = maxbo.generation;
        if (sire.generation > maxbo.generation) {
            parentGen = sire.generation;
        }

        // Call the sooper-sekret gene mixing operation.
        uint256 childGenes = geneScience.mixGenes(maxbo.genes, sire.genes, maxbo.cooldownEndBlock - 1);

        // Make the new kitten!
        address owner = kittyIndexToOwner[_maxboId];
        uint256 kittenId = _createKitty(_maxboId, maxbo.siringWithId, parentGen + 1, childGenes, owner);

        // Clear the reference to sire from the maxbo (REQUIRED! Having siringWithId
        // set is what marks a maxbo as being pregnant.)
        delete maxbo.siringWithId;
...
contract KittyAuction is KittyBreeding {
    ...
    ///  Immediately breeds the winning maxbo with the sire on auction.
    /// @param _sireId - ID of the sire on auction.
    /// @param _maxboId - ID of the maxbo owned by the bidder.
        ...
        whenNotPaused
    {
        // Auction contract checks input sizes
        require(_owns(msg.sender, _maxboId));
        require(isReadyToBreed(_maxboId));
        require(_canBreedWithViaAuction(_maxboId, _sireId));
        ...
        _breedWith(uint32(_maxboId), uint32(_sireId));
    }
