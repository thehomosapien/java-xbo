package stest.xbo.wallet.fulltest;
...
public class XBODice {
  ...
  public void xboDice() {
    ECKey ecKey1 = new ECKey(Utils.getRandom());
    byte[] xboDiceAddress = ecKey1.getAddress();
    String xboDiceKey = ByteArray.toHexString(ecKey1.getPrivKeyBytes());
    PublicMethed
        .sendcoin(xboDiceAddress, 100000000000L, fromAddress, testKey002, blockingStubFull);
    String contractName = "XBODice";
    String code = Configuration.getByPath("testng.conf")
        .getString("code.code_XBODice_xboDice");
    String abi = Configuration.getByPath("testng.conf")
        .getString("abi.abi_XBODice_xboDice");
    byte[] contractAddress = PublicMethed.deployContract(contractName, abi, code, "",
        maxFeeLimit, 1000000000L, 100, null, xboDiceKey, xboDiceAddress, blockingStubFull);
    ...
    for (Integer i = 0; i < 100; i++) {
      ...
      txid = PublicMethed.triggerContract(contractAddress,
          ...
          1000000, maxFeeLimit, xboDiceAddress, xboDiceKey, blockingStubFull);
      logger.info(txid);
      txidList.add(txid);
