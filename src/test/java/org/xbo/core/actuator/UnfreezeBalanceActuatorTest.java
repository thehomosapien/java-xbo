...
public class UnfreezeBalanceActuatorTest {
  ...
  private Any getContractForCpu(String ownerAddress) {
    return Any.pack(
        Contract.UnfreezeBalanceContract.newBuilder()
            .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(ownerAddress)))
            .setResource(org.xbo.protos.Contract.ResourceCode.ENERGY)
            .build());
  }
  ...
  public void testUnfreezeBalanceForBandwidth() {
    ...
    Assert.assertEquals(accountCapsule.getXBOPower(), frozenBalance);
    ...
    try {
      ...
      Assert.assertEquals(owner.getXBOPower(), 0L);
  ...
  public void testUnfreezeDelegatedBalanceForBandwidth() {
    ...
    Assert.assertEquals(frozenBalance, owner.getXBOPower());
    ...
    Assert.assertEquals(0L, receiver.getXBOPower());
    ...
    try {
      ...
      Assert.assertEquals(0L, ownerResult.getXBOPower());
  ...
  public void testUnfreezeDelegatedBalanceForBandwidthWithRecreatedReceiver() {
    ...
    try {
      ...
      Assert.assertEquals(e.getMessage(),"AcquiredDelegatedFrozenBalanceForBandwidth[10] < delegatedBandwidth[1000000000]");
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
    }
  ...
  public void testUnfreezeDelegatedBalanceForCpuWithRecreatedReceiver() {
    ...
    try {
      ...
      Assert.assertEquals(e.getMessage(),"AcquiredDelegatedFrozenBalanceForEnergy[10] < delegatedEnergy[1000000000]");
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
    }
