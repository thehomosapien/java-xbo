...
import org.xbo.core.config.Parameter;
...
import org.xbo.protos.Contract.AssetIssueContract.FrozenSupply;
...
public class AssetIssueActuatorTest {
  ...
  public void SameTokenNameCloseAssetIssueSuccess() {
    ...
    try {
      ...
      Assert.assertEquals(XBO_NUM, assetIssueCapsule.getTrxNum());
      ...
      Assert.assertEquals(XBO_NUM, assetIssueCapsuleV2.getTrxNum());
  ...
  @Test
  public void assetIssueXBONameTest() {
    dbManager.getDynamicPropertiesStore().saveAllowSameTokenName(1);
    Any contract = Any.pack(Contract.AssetIssueContract.newBuilder()
        .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)))
        .setName(ByteString.copyFromUtf8("XBO"))
        .setTotalSupply(TOTAL_SUPPLY)
        .setTrxNum(XBO_NUM).setNum(NUM)
        .setStartTime(startTime)
        .setEndTime(endTime)
    ...
    try {
      ...
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("assetName can't be xbo", e.getMessage());
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
    } finally {
      dbManager.getAssetIssueStore().delete(ByteArray.fromString(NAME));
