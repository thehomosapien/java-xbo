...
import org.xbo.common.runtime.TVMTestResult;
...
//     deploy contract
    ...
    byte[] addressWithSufficientBalance = Wallet.generateContractAddress(xbo);
    runtime = TvmTestUtils.processTransactionAndReturnRuntime(xbo, rootDeposit, null);
    ...
    xbo = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, ABI, code, 0, fee, consumeUserResourcePercent, null);
    byte[] addressWithoutBalance = Wallet.generateContractAddress(xbo);
    runtime = TvmTestUtils.processTransactionAndReturnRuntime(xbo, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
