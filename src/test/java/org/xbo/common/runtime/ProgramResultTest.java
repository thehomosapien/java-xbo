...
public class ProgramResultTest {
  ...
  public void successAndFailResultTest()
    ...
    TransactionTrace traceSuccess = TvmTestUtils
        .processTransactionAndReturnTrace(xbo1, deposit, null);
    ...
    checkTransactionInfo(traceSuccess, xbo1, null, internalTransactionsList);
    ...
    byte[] triggerData2 = TvmTestUtils.parseAbi("transfer(address,bool)",
        params);
    Transaction xbo2 = TvmTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), aContract,
            triggerData2, 0, 100000000);
    TransactionTrace traceFailed = TvmTestUtils
        .processTransactionAndReturnTrace(xbo2, deposit, null);
    runtime = traceFailed.getRuntime();
    byte[] bContract2 = Wallet
        .generateContractAddress(new TransactionCapsule(xbo2).getTransactionId().getBytes(), 0);
    ...
    checkTransactionInfo(traceFailed, xbo2, null, internalTransactionsListFail);
  }
  ...
  public void suicideResultTest()
    ...
    byte[] triggerData1 = TvmTestUtils.parseAbi("suicide(address)",
        params);
    Transaction xbo = TvmTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), suicideContract,
            triggerData1, 0, 100000000);
    TransactionTrace trace = TvmTestUtils.processTransactionAndReturnTrace(xbo, deposit, null);
    ...
    Assert.assertEquals(internalTransactionsList.get(0).isRejected(), false);
    checkTransactionInfo(trace, xbo, null, internalTransactionsList);
  }
  ...
  public void checkTransactionInfo(TransactionTrace trace, Transaction xbo, BlockCapsule block,
      List<InternalTransaction> internalTransactionsList) {
    TransactionInfoCapsule xboInfoCapsule = TransactionInfoCapsule
        .buildInstance(new TransactionCapsule(xbo), null, trace);
    List<Protocol.InternalTransaction> internalTransactionListFromProtocol = xboInfoCapsule
