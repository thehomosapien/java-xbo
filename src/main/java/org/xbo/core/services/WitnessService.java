...
import static org.xbo.core.witness.BlockProductionCondition.NOT_MY_TURN;
...
import org.xbo.common.backup.BackupServer;
...
import org.xbo.core.witness.BlockProductionCondition;
...
public class WitnessService implements Service {
   ...
   * Construction method.
   */
  public WitnessService(Application xboApp, XBOApplicationContext context) {
    this.xboApp = xboApp;
    this.context = context;
    backupManager = context.getBean(BackupManager.class);
    backupServer = context.getBean(BackupServer.class);
    xboNetService = context.getBean(XBONetService.class);
    generateThread = new Thread(scheduleProductionLoop);
    manager = xboApp.getDbManager();
  ...
  private BlockProductionCondition tryProduceBlock() throws InterruptedException {
    ...
    if (this.needSyncCheck) {
      ...
      if (nexSlotTime > now) { // check sync during first loop
        ...
        logger.debug("Not sync ,now:{},headBlockTime:{},headBlockNumber:{},headBlockId:{}",
            new DateTime(now),
            new DateTime(this.xboApp.getDbManager().getDynamicPropertiesStore()
                .getLatestBlockHeaderTimestamp()),
            this.xboApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderNumber(),
            this.xboApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderHash());
    ...
    try {
      ...
      synchronized (xboApp.getDbManager()) {
        ...
        if (slot == 0) {
          logger.info("Not time yet,now:{},headBlockTime:{},headBlockNumber:{},headBlockId:{}",
              new DateTime(now),
              new DateTime(
                  this.xboApp.getDbManager().getDynamicPropertiesStore()
                      .getLatestBlockHeaderTimestamp()),
              this.xboApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderNumber(),
              this.xboApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderHash());
        ...
        if (DateTime.now().getMillis() - now > timeout) {
          logger.warn("Task timeout ( > {}ms)，startTime:{},endTime:{}", timeout, new DateTime(now),
              DateTime.now());
          xboApp.getDbManager().eraseBlock();
  ...
  private void broadcastBlock(BlockCapsule block) {
    try {
      xboNetService.broadcast(new BlockMessage(block.getData()));
    } catch (Exception ex) {
  ...
  private BlockCapsule generateBlock(long when, ByteString witnessAddress,
    ...
    return xboApp.getDbManager().generateBlock(this.localWitnessStateMap.get(witnessAddress), when,
  ...
  public void init() {
    ...
    WitnessCapsule witnessCapsule = this.xboApp.getDbManager().getWitnessStore()
