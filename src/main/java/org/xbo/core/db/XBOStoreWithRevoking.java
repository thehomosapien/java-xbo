...
import org.xbo.core.db2.common.LevelDB;
import org.xbo.core.db2.common.RocksDB;
import org.xbo.core.db2.core.IXBOChainBase;
import org.xbo.core.db2.core.RevokingDBWithCachingNewValue;
import org.xbo.core.db2.core.RevokingDBWithCachingOldValue;
import org.xbo.core.exception.BadItemException;
import org.xbo.core.exception.ItemNotFoundException;

@Slf4j(topic = "DB")
public abstract class XBOStoreWithRevoking<T extends ProtoCapsule> implements IXBOChainBase<T> {
  ...
  protected XBOStoreWithRevoking(String dbName) {
    this.dbName = dbName;
  ...
  protected XBOStoreWithRevoking(String dbName, Class<? extends DB> clz) {
    this.dbName = dbName;
    int dbVersion = Args.getInstance().getStorage().getDbVersion();
    if (dbVersion == 2) {
  ...
  // only for test
  protected XBOStoreWithRevoking(String dbName, RevokingDatabase revokingDatabase) {
    this.revokingDB = new RevokingDBWithCachingOldValue(dbName,
