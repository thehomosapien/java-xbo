...
import static org.xbo.protos.Protocol.Transaction.Contract.ContractType.ShieldedTransferContract;
...
public class BandwidthProcessor extends ResourceProcessor {
  ...
  @Override
  public void consume(TransactionCapsule xbo, TransactionTrace trace)
      throws ContractValidateException, AccountResourceInsufficientException, TooBigTransactionResultException {
    List<Contract> contracts = xbo.getInstance().getRawData().getContractList();
    if (xbo.getResultSerializedSize() > Constant.MAX_RESULT_SIZE_IN_TX * contracts.size()) {
    ...
    if (dbManager.getDynamicPropertiesStore().supportVM()) {
      bytesSize = xbo.getInstance().toBuilder().clearRet().build().getSerializedSize();
    } else {
      bytesSize = xbo.getSerializedSize();
    }
    ...
    for (Contract contract : contracts) {
      ...
      logger.debug("xboId {},bandwidth cost :{}", xbo.getTransactionId(), bytesSize);
