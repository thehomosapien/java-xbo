...
import static org.xbo.common.runtime.utils.MUtil.transferToken;
...
import org.xbo.common.runtime.vm.LogInfoTriggerParser;
import org.xbo.common.runtime.vm.VM;
import org.xbo.common.runtime.vm.VMConstant;
import org.xbo.common.runtime.vm.VMUtils;
import org.xbo.common.runtime.vm.program.InternalTransaction;
import org.xbo.common.runtime.vm.program.InternalTransaction.ExecutorType;
...
import org.xbo.common.runtime.vm.program.ProgramPrecompile;
...
public class RuntimeImpl implements Runtime {
  ...
  public RuntimeImpl(TransactionTrace trace, BlockCapsule block, Deposit deposit,
    ...
    ContractType contractType = this.xbo.getRawData().getContract(0).getType();
    switch (contractType.getNumber()) {
      case ContractType.TriggerSmartContract_VALUE:
        xboType = TrxType.XBO_CONTRACT_CALL_TYPE;
        break;
      case ContractType.CreateSmartContract_VALUE:
        xboType = TrxType.XBO_CONTRACT_CREATION_TYPE;
        break;
      default:
        xboType = TrxType.XBO_PRECOMPILED_TYPE;
    }
  }


  /**
   * For constant xbo with latest blockCap.
   */
  ...
  private void precompiled() throws ContractValidateException, ContractExeException {
    ...
    final List<Actuator> actuatorList = ActuatorFactory
        .createActuator(xboCap, deposit.getDbManager());
  ...
  public long getTotalEnergyLimitWithFixRatio(AccountCapsule creator, AccountCapsule caller,
    ...
    if (Arrays.equals(creator.getAddress().toByteArray(), caller.getAddress().toByteArray())) {
      ...
      // which will cost the feeLimit XBO when the creator's frozen energy is 0.
      return callerEnergyLimit;
    }
  ...
  private double getCpuLimitInUsRatio() {
    ...
    if (ExecutorType.ET_NORMAL_TYPE == executorType) {
      ...
      if (this.blockCap != null && blockCap.generatedByMyself &&
        ...
        if (xbo.getRet(0).getContractRet() == contractResult.OUT_OF_TIME) {
  ...
  private void create()
    ...
    byte[] contractAddress = Wallet.generateContractAddress(xbo);
    ...
    try {
      long feeLimit = xbo.getRawData().getFeeLimit();
      ...
      rootInternalTransaction = new InternalTransaction(xbo, xboType);
      ...
      ProgramInvoke programInvoke = programInvokeFactory
          .createProgramInvoke(TrxType.XBO_CONTRACT_CREATION_TYPE, executorType, xbo,
      ...
      this.program = new Program(ops, programInvoke, rootInternalTransaction, config,
          this.blockCap);
      byte[] txId = new TransactionCapsule(xbo).getTransactionId().getBytes();
  ...
  private void call()
    ...
    Contract.TriggerSmartContract contract = ContractCapsule.getTriggerContractFromTransaction(xbo);
    if (contract == null) {
      return;
    }
    ...
    if (isNotEmpty(code)) {
      ...
      long vmShouldEndInUs = vmStartInUs + thisTxCPULimitInUs;
      ProgramInvoke programInvoke = programInvokeFactory
          .createProgramInvoke(TrxType.XBO_CONTRACT_CALL_TYPE, executorType, xbo,
              tokenValue, tokenId, blockCap.getInstance(), deposit, vmStartInUs,
              vmShouldEndInUs, energyLimit);
      if (isStaticCall) {
  ...
  public void go() {
    try {
      if (vm != null) {
        TransactionCapsule xboCap = new TransactionCapsule(xbo);
        if (null != blockCap && blockCap.generatedByMyself && null != xboCap.getContractRet()
            && contractResult.OUT_OF_TIME == xboCap.getContractRet()) {
        ...
        if (isStaticCall) {
          long callValue = TransactionCapsule.getCallValue(xbo.getRawData().getContract(0));
          long callTokenValue = TransactionCapsule
              .getCallTokenValue(xbo.getRawData().getContract(0));
        ...
        if (TrxType.XBO_CONTRACT_CREATION_TYPE == xboType && !result.isRevert()) {
  ...
  public void finalization() {
    if (StringUtils.isEmpty(runtimeError)) {
      for (DataWord contract : result.getDeleteAccounts()) {
        deposit.deleteContract(convertToXBOAddress((contract.getLast20Bytes())));
      }
    }
