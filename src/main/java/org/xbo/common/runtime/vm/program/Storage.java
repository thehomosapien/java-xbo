...
import org.xbo.core.db.StorageRowStore;
...
public class Storage {
  ...
  public void generateAddrHash(byte[] xboId) {
    // update addreHash for create2
    addrHash = addrHash(address, xboId);
  }
  ...
  private static byte[] addrHash(byte[] address, byte[] xboHash) {
    if (ByteUtil.isNullOrZeroArray(xboHash)) {
      return Hash.sha3(address);
    }
    return Hash.sha3(ByteUtil.merge(address, xboHash));
  }
