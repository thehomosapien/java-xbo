...
package org.xbo.common.runtime.vm.program;
...
import static org.xbo.common.runtime.utils.MUtil.convertToXBOAddress;
import static org.xbo.common.runtime.utils.MUtil.transfer;
import static org.xbo.common.runtime.utils.MUtil.transferAllToken;
import static org.xbo.common.utils.BIUtil.isPositive;
import static org.xbo.common.utils.BIUtil.toBI;
import static org.xbo.common.utils.ByteUtil.stripLeadingZeroes;
...
import org.xbo.common.runtime.vm.DataWord;
import org.xbo.common.runtime.vm.EnergyCost;
import org.xbo.common.runtime.vm.MessageCall;
import org.xbo.common.runtime.vm.OpCode;
import org.xbo.common.runtime.vm.PrecompiledContracts;
import org.xbo.common.runtime.vm.VM;
import org.xbo.common.runtime.vm.VMConstant;
import org.xbo.common.runtime.vm.program.invoke.ProgramInvoke;
import org.xbo.common.runtime.vm.program.invoke.ProgramInvokeFactory;
import org.xbo.common.runtime.vm.program.invoke.ProgramInvokeFactoryImpl;
import org.xbo.common.runtime.vm.program.listener.CompositeProgramListener;
import org.xbo.common.runtime.vm.program.listener.ProgramListenerAware;
import org.xbo.common.runtime.vm.program.listener.ProgramStorageChangeListener;
import org.xbo.common.runtime.vm.trace.ProgramTrace;
import org.xbo.common.runtime.vm.trace.ProgramTraceListener;
import org.xbo.common.storage.Deposit;
import org.xbo.common.utils.ByteUtil;
import org.xbo.common.utils.FastByteComparisons;
...
import org.xbo.protos.Protocol.SmartContract.Builder;
...
public class Program {
  ...
  /**
   * @param transferAddress the address send xbo to.
   * @param value the xbo value transferred in the internaltransaction
   */
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = convertToXBOAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = convertToXBOAddress(obtainerAddress.getLast20Bytes());
      ...
      try {
        ...
        if (VMConfig.allowTvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all xbo failed in suicide: %s", e.getMessage());
        }
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
      boolean isCreate2) {
    byte[] senderAddress = convertToXBOAddress(this.getContractAddress().getLast20Bytes());
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(convertToXBOAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = convertToXBOAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = convertToXBOAddress(getContractAddress().getLast20Bytes());
    ...
    if (byTestingSuite()) {
      ...
      if (!isTokenTransfer) {
        try {
          ...
          if (VMConfig.allowTvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), "refund energy from message call");
            throw new TransferException("transfer xbo failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
        deposit.addBalance(senderAddress, -endowment);
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(convertToXBOAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(convertToXBOAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] xboAddr = convertToXBOAddress(address.getLast20Bytes());
    AccountCapsule account = getContractState().getAccount(xboAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(xboAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(xboAddr, contract);
        }
      } else {
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState().getBalance(convertToXBOAddress(address.getLast20Bytes()));
    return new DataWord(balance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
        .getContract(convertToXBOAddress(address.getLast20Bytes()));
  ...
  public DataWord storageLoad(DataWord key) {
    DataWord ret = getContractState()
        .getStorageValue(convertToXBOAddress(getContractAddress().getLast20Bytes()), key.clone());
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState().getTokenBalance(convertToXBOAddress(address.getLast20Bytes()),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    byte[] senderAddress = convertToXBOAddress(this.getCallerAddress().getLast20Bytes());
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(convertToXBOAddress(msg.getType().callIsDelegate()
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(XBOException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(XBOException e) {
      return new PrecompiledContractException(e.getMessage());
    }
