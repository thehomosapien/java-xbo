...
public class InternalTransaction {
  ...
  /* the amount of xbo to transfer (calculated as sun) */
  private long value;
  ...
  public enum TrxType {
    XBO_PRECOMPILED_TYPE,
    XBO_CONTRACT_CREATION_TYPE,
    XBO_CONTRACT_CALL_TYPE,
    XBO_UNKNOWN_TYPE,
  }
   ...
   */
  public InternalTransaction(Transaction xbo, InternalTransaction.TrxType xboType)
      throws ContractValidateException {
    this.transaction = xbo;
    TransactionCapsule xboCap = new TransactionCapsule(xbo);
    this.protoEncoded = xboCap.getData();
    this.nonce = 0;
    ...
    this.deep = -1;
    if (xboType == TrxType.XBO_CONTRACT_CREATION_TYPE) {
      ...
      this.transferToAddress = Wallet.generateContractAddress(xbo);
      this.note = "create";
    ...
    } else if (xboType == TrxType.XBO_CONTRACT_CALL_TYPE) {
      TriggerSmartContract contract = ContractCapsule.getTriggerContractFromTransaction(xbo);
      if (contract == null) {
    ...
    this.hash = xboCap.getTransactionId().getBytes();
  }
  ...
  public InternalTransaction(byte[] parentHash, int deep, int index,
    ...
    // in this case, value also can represent a tokenValue when tokenId is not null, otherwise it is a xbo callvalue.
    this.value = value;
    this.data = ArrayUtils.nullToEmpty(data);
    this.nonce = nonce;
    this.hash = getHash();
    // in a contract call contract case, only one value should be used. xbo or a token. can't be both. We should avoid using
